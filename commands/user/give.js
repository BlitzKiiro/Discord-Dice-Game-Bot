...
module.exports = {
  ...
  async execute(message, database, args) {
    ...
    if (!(message.mentions.members.size > 0) || args === null) {
      await message.reply(
        `Please provide a mention and a number at this form\n ${prefix} ${this.name} @mention amount `
      );
      return;
    }
    if (message.mentions.users.first().bot === true) {
      const meme = new MessageAttachment(
        `${__root}/attachments/meme replies/stupid fellas.jpg`
      );
      await message.reply(
    ...
    if (message.mentions.users.first() === message.author) {
      const meme = new MessageAttachment(
        `${__root}/attachments/meme replies/confused.jpg`
      );
      await message.reply(
        "Dude! \n You are literally trying to give money to yourself",
        meme
      );
      return;
    }
    ...
    const meme = new MessageAttachment(
      `${__root}/attachments/meme replies/money.jpg`
    );
