...
module.exports = class Database {
  ...
  getPlayer = async (uid, sid) => {
    try {
      ...
      const record =
        (await this.prisma.games_Record.findUnique({
          where: {
            uid_sid: { uid, sid },
          },
          select: {
  ...
  getServerStats = async (sid) => {
    try {
      ...
      const players =
        (await this.prisma.games_Record.count({
          where: {
            sid,
            played: {
              gt: 0,
  ...
  getTopServerWinners = async (sid) => {
    const players_list = await this.prisma.games_Record.findMany({
      where: { sid },
      orderBy: [
        {
          won: "desc",
